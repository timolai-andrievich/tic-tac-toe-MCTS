...
def models_play(
    nn1: NN, nn2: NN, config: Config, first_starts: bool, action_choice="best"
):
...
def models_match(nn1: NN, nn2: NN, config: Config, action_choice="best"):
    ...
    for i in tqdm.tqdm(range(config.test_games)):
        winner = models_play(
            nn1, nn2, config, first_starts=i % 2 == 0, action_choice=action_choice
        )
        res[winner] += 1
    return res
...
def play_and_visualize(nn1: NN, nn2: NN, config: Config, action_choice="best"):
    ...
    try:
        while not game.is_terminal():
            ...
            print(
                f"Making move {action} with probability {probs[action] * 100:.2f}%"
            ...
            print(
                f"{(raw_probs.reshape((Game.board_height, Game.board_width)) * 100).round(2)}"
            )
            trees[0].commit_action(action)
            trees[1].commit_action(action)
            game.commit_action(action)
            ...
            c = pos[
                action // Game.board_height * (Game.board_height + 1)
                + action % Game.board_height
            ]
...
def play_game_against_random(
    nn: NN, config: Config, first_starts: bool, action_choice: str = "best"
):
    assert action_choice in action_choices
    game = Game().copy()
    tree = MCTS(config)
...
def evaluate_model_against_random(nn: NN, config: Config, action_choice="best"):
    ...
    for i in tqdm.tqdm(range(config.test_games)):
        results[
            play_game_against_random(nn, config, i % 2, action_choice=action_choice)
        ] += 1
    return results
...
def evaluate_models_against_random(games: int, config: Config, action_choice="best"):
    ...
    for name, model in models.items():
        models_results[name] = evaluate_model_against_random(
            model, config, action_choice=action_choice
        )
    sorted_models = list(
        sorted(models_results.items(), key=lambda x: x[1][-1] - x[1][1])
    )
    for name, (t, w, l) in sorted_models:
        ...
        print(
            f"{name:>30}: +{w}-{l}={t}, score: {(w + t / 2) / games}, elo: {lb:.0f} - {av:.0f} - {ub:.0f}"
        )


def evaluate_models_against_nn(nn: NN, config: Config, action_choice="best"):
    assert action_choice in action_choices
    print("calculating base elo...")
    base_results = evaluate_model_against_random(
        nn, config, action_choice=action_choice
    )
    base_lb, base_av, base_ub = calculate_distribution(
        base_results[1] + base_results[0] / 2, base_results[-1] + base_results[0] / 2
    )
    files = glob.glob("../models/*")
    models_list: List[str] = []
    models_results: Dict[str, List[int, int, int]] = {}
    ...
    for file_path in files:
        ...
        models_results[file_path] = [0, 0, 0]
    for name, model in models.items():
        models_results[name] = models_match(
            nn, model, config, action_choice=action_choice
        )
    sorted_models = list(
        sorted(models_results.items(), key=lambda x: x[1][-1] - x[1][1])
    )
    ...
    for name, (t, w, l) in sorted_models:
        lb, av, ub = calculate_distribution(w + t / 2, l + t / 2)
        print(
            f"{name:>30}: +{w}-{l}={t}, score: {(w + t / 2) / config.test_games}, elo: {lb:.0f} - {av:.0f} - {ub:.0f}"
        )


def play_against_minmax(
    nn: NN,
    config: Config,
    first_starts: bool,
    action_choice: str = "best",
    mm: MinMax = None,
):
    assert action_choice in action_choices
    if mm is None:
...
def evaluate_model_against_minmax(
    nn: NN, config: Config, action_choice="best", mm: MinMax = None
):
    results = [0, 0, 0]
    if mm is None:
        mm = MinMax()
    for i in tqdm.tqdm(range(config.test_games)):
        results[
            play_against_minmax(nn, config, i % 2, action_choice=action_choice, mm=mm)
        ] += 1
    return results
...
def evaluate_models_against_minmax(config: Config, action_choice="best"):
    ...
    for file_path in files:
        ...
        models_results[file_path] = [0, 0, 0]
    for name, model in models.items():
        models_results[name] = evaluate_model_against_minmax(
            model, config, action_choice=action_choice, mm=mm
        )
    sorted_models = list(
        sorted(models_results.items(), key=lambda x: x[1][-1] - x[1][1])
    )
...
def play_and_visualize_against_minmax(
    nn: NN,
    first_starts: bool,
    config: Config,
    mm: MinMax = None,
